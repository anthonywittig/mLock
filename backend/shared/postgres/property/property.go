...
func GetByID(ctx context.Context, id string) (shared.Property, bool, error) {
	db, err := postgres.GetDB(ctx)
	if err != nil {
		return shared.Property{}, false, fmt.Errorf("error getting DB: %s", err.Error())
	}

	// Verify id is a uuid.
	parsedID, err := uuid.Parse(id)
	if err != nil {
		return shared.Property{}, false, fmt.Errorf("error parsing ID: %s", err.Error())
	}
	...
	if err := row.Scan(&idResult, &nameResult, &createdByResult); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return shared.Property{}, false, nil // Not really an error.
		}

		return shared.Property{}, false, fmt.Errorf("error scanning row: %s", err.Error())
	}

	return shared.Property{ID: idResult, Name: nameResult, CreatedBy: createdByResult}, true, nil
}

func GetByName(ctx context.Context, name string) (shared.Property, bool, error) {
...
func GetAll(ctx context.Context) ([]shared.Property, error) {
	db, err := postgres.GetDB(ctx)
	if err != nil {
		return []shared.Property{}, fmt.Errorf("error getting DB: %s", err.Error())
	}

	rows, err := db.QueryContext(ctx, "SELECT id, name, created_by FROM properties ORDER BY name")
	if err != nil {
		return []shared.Property{}, fmt.Errorf("error doing query: %s", err.Error())
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &name, &createdBy); err != nil {
			return []shared.Property{}, fmt.Errorf("error scanning row: %s", err.Error())
		}
		entities = append(entities, shared.Property{ID: id, Name: name, CreatedBy: createdBy})
	}
...
func Insert(ctx context.Context, name string) (shared.Property, error) {
	name = strings.TrimSpace(name)
	...
	if err != nil {
		return shared.Property{}, fmt.Errorf("error inserting into DB: %s", err.Error())
	}
